...
def __atualiza_mach(f_atv):
    ...
    # senão, acelerando...ultrapassou a demanda ?
    elif f_atv.f_atv_vel_mac_atu > f_atv.f_atv_vel_mac_dem:
...
def __atualiza_velocidade(f_atv, ff_delta_t):
    ...
    if f_atv.f_trf_vel_atu != f_atv.f_atv_vel_dem:
        ...
        # elif f_atv.f_trf_vel_atu < f_atv.f_atv_vel_dem:
            ...
            # obtém a aceleração (performance)
            # f_atv.f_atv_acel = f_atv.ptr_trf_prf.f_prf_raz_var_vel
            # M_LOG.debug("aceleração vôo: " + str(f_atv.f_atv_acel))

        # senão, está freando
        # else:
            # obtém a desaceleração (performance)
            # f_atv.f_atv_acel = f_atv.ptr_trf_prf.f_prf_desacel_crz
            # M_LOG.debug("desaceleração vôo: " + str(f_atv.f_atv_acel))
        ...
        if f_atv.f_trf_vel_atu < f_atv.f_atv_vel_dem:
            ...
            # M_LOG.debug("__atualiza_velocidade:nova velocidade (A): " + str(f_atv.f_trf_vel_atu))
        ...
        # está freando ?
            ...
            # diminui a velocidade da aeronave (v = vo - at)
            f_atv.f_trf_vel_atu -= f_atv.f_atv_acel * ff_delta_t
            # M_LOG.debug("__atualiza_velocidade:nova velocidade (D): " + str(f_atv.f_trf_vel_atu))
...
def dados_dinamicos(f_atv, f_cine_data, ff_delta_t, f_stime):
    ...
    if (not f_atv.v_atv_ok) or (ldefs.E_ATIVA != f_atv.en_trf_est_atv):
        ...
        # logger
        # M_LOG.info("dados_dinamicos:<E01: aeronave não ativa.")
    ...
    # senão, registra a falha no arquivo de log
    else:
        # logger
